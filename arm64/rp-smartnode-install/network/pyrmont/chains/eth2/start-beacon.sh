...
if [ "$CLIENT" = "nimbus" ]; then
    ...
    # Run
    exec /home/user/nimbus-eth2/build/nimbus_beacon_node --max-peers=80 --non-interactive --enr-auto-update --network=pyrmont --data-dir=/ethclient/nimbus --log-file="/ethclient/nimbus/nbc_bn_$(date +"%Y%m%d%H%M%S").log" --tcp-port=9001 --udp-port=9001 \
