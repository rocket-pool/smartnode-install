...
if [ "$CLIENT" = "lighthouse" ]; then
    ...
    CMD="ionice -c 2 -n 0 /usr/local/bin/lighthouse beacon --network mainnet --datadir /ethclient/lighthouse --port $ETH2_P2P_PORT --discovery-port $ETH2_P2P_PORT --eth1 --eth1-endpoints $ETH1_ENDPOINTS --http --http-address 0.0.0.0 --http-port 5052 --eth1-blocks-per-log-query 150 --disable-upnp"
...
if [ "$CLIENT" = "prysm" ]; then
    ...
    CMD="ionice -c 2 -n 0 /app/cmd/beacon-chain/beacon-chain --accept-terms-of-use --mainnet --datadir /ethclient/prysm --p2p-tcp-port $ETH2_P2P_PORT --p2p-udp-port $ETH2_P2P_PORT --http-web3provider $ETH1_PROVIDER $FALLBACK_PROVIDER --rpc-host 0.0.0.0 --rpc-port $ETH2_RPC_PORT --grpc-gateway-host 0.0.0.0 --grpc-gateway-port 5052 --eth1-header-req-limit 150"
...
if [ "$CLIENT" = "teku" ]; then
    ...
    CMD="ionice -c 2 -n 0 /opt/teku/bin/teku --network=mainnet --data-path=/ethclient/teku --p2p-port=$ETH2_P2P_PORT --eth1-endpoints=$ETH1_ENDPOINTS --rest-api-enabled --rest-api-interface=0.0.0.0 --rest-api-port=5052 --rest-api-host-allowlist=* --eth1-deposit-contract-max-request-size=150 --log-destination=CONSOLE"
